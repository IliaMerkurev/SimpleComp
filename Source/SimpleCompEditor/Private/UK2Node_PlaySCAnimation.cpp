...
const FName UK2Node_PlaySCAnimation::PN_ReverseFromCurrent(TEXT("ReverseFromCurrent"));
...
UK2Node_PlaySCAnimation::UK2Node_PlaySCAnimation()
{
...
void UK2Node_PlaySCAnimation::AllocateDefaultPins()
{
    ...
    UEdGraphPin* ComponentPin =
        CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, USCCurveAnimComponent::StaticClass(), PN_Component);
    K2Schema->ConstructBasicPinTooltip(*ComponentPin,
        NSLOCTEXT("K2Node", "ComponentTooltip", "The animation component to play on"), ComponentPin->PinToolTip);

    UEdGraphPin* SequencePin =
        CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, USCAnimSequence::StaticClass(), PN_Sequence);
    ...
    UEdGraphPin* LoopPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Boolean, PN_Loop);
    LoopPin->DefaultValue = TEXT("false");
    K2Schema->ConstructBasicPinTooltip(*LoopPin, NSLOCTEXT("K2Node", "LoopTooltip", "Whether to loop the animation"),
        LoopPin->PinToolTip);
...
void UK2Node_PlaySCAnimation::RemoveNotifyPins()
{
    ...
    for (UEdGraphPin* Pin : Pins)
    {
        if (Pin->Direction == EGPD_Output && Pin->PinType.PinCategory == UEdGraphSchema_K2::PC_Exec &&
            Pin->PinName != PN_Then && Pin->PinName != PN_Update && Pin->PinName != PN_Finished)
        {
...
#if WITH_EDITOR
void UK2Node_PlaySCAnimation::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
FText UK2Node_PlaySCAnimation::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
    return NSLOCTEXT("K2Node", "PlaySCAnimation_Title", "Play Simple Animation");
}
...
void UK2Node_PlaySCAnimation::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
    ...
    const TArray<FActionPath> Paths = {{PN_Play, TEXT("Play"), false, true, false},
        {PN_PlayFromStart, TEXT("Play"), false, true, true}, {PN_Stop, TEXT("Stop"), true, false, false},
        {PN_Pause, TEXT("Pause"), true, false, false}, {PN_Resume, TEXT("Resume"), true, false, false},
        {PN_ReverseFromEnd, TEXT("ReverseFromEnd"), false, true, true},
        {PN_ReverseFromCurrent, TEXT("ReverseFromCurrent"), false, false, false}};

    auto ConnectInternalOutputToUserOutput = [&](UEdGraphPin* InternalOut, const FName& NodeOutputName)
    {
        if (!InternalOut)
            return;
    ...
    for (const FActionPath& Path : Paths)
    {
        ...
        CreateProxyNode->FunctionReference.SetExternalMember(GET_FUNCTION_NAME_CHECKED(USCAnimAsyncAction, CreateProxy),
        ...
        CompilerContext.MovePinLinksToIntermediate(*ExecInput, *CreateProxyNode->GetExecPin());
        ...
        if (!Path.bIsControl)
        {
                                     ...
                                     const FName& ParamName2 = NAME_None, const FName& NodeParamName2 = NAME_None)
            {
                ...
                AddDel->SetFromProperty(USCAnimAsyncAction::StaticClass()->FindPropertyByName(DelegateName), false,
                    USCAnimAsyncAction::StaticClass());
                ...
                Evt->CustomFunctionName = *FString::Printf(TEXT("%s_%s_%s"), *GetName(), *DelegateName.ToString(),
                    *FGuid::NewGuid().ToString());

                if (FMulticastDelegateProperty* Prop = CastField<FMulticastDelegateProperty>(
                ...
                {
                    DelPin->MakeLinkTo(Evt->FindPin(UK2Node_CustomEvent::DelegateOutputName));
                }
            ...
            UK2Node_AddDelegate* AddDelNotify =
                CompilerContext.SpawnIntermediateNode<UK2Node_AddDelegate>(this, SourceGraph);
            AddDelNotify->SetFromProperty(USCAnimAsyncAction::StaticClass()->FindPropertyByName(TEXT("OnNotify")),
                false, USCAnimAsyncAction::StaticClass());
            AddDelNotify->AllocateDefaultPins();
            if (UEdGraphPin* Self = AddDelNotify->FindPin(UEdGraphSchema_K2::PN_Self))
            {
                Self->MakeLinkTo(ProxyPin);
            }
            ...
            UK2Node_CustomEvent* EvtNotify =
                CompilerContext.SpawnIntermediateNode<UK2Node_CustomEvent>(this, SourceGraph);
            EvtNotify->CustomFunctionName =
                *FString::Printf(TEXT("%s_OnNotify_%s"), *GetName(), *FGuid::NewGuid().ToString());
            if (FMulticastDelegateProperty* Prop = CastField<FMulticastDelegateProperty>(
                    USCAnimAsyncAction::StaticClass()->FindPropertyByName(TEXT("OnNotify"))))
            {
            ...
            {
                DelPin->MakeLinkTo(EvtNotify->FindPin(UK2Node_CustomEvent::DelegateOutputName));
            }

            LastNode->FindPin(UEdGraphSchema_K2::PN_Then)->MakeLinkTo(AddDelNotify->GetExecPin());
            ...
            if (UEdGraphPin* Self = ActivateNode->FindPin(UEdGraphSchema_K2::PN_Self))
            {
                Self->MakeLinkTo(ProxyPin);
            }
            LastNode->FindPin(UEdGraphSchema_K2::PN_Then)->MakeLinkTo(ActivateNode->GetExecPin());
            LastNode = ActivateNode;
        }

        UK2Node_CallFunction* CallFuncNode =
            CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(this, SourceGraph);
        CallFuncNode->FunctionReference.SetExternalMember(Path.FuncName, USCAnimAsyncAction::StaticClass());
        ...
        LastNode->FindPin(UEdGraphSchema_K2::PN_Then)->MakeLinkTo(CallFuncNode->GetExecPin());
...
FSlateIcon UK2Node_PlaySCAnimation::GetIconAndTint(FLinearColor& OutColor) const
{
...
void UK2Node_PlaySCAnimation::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
